...
parser.add_argument('app', type = str.lower, choices = [ 'kandan', 'redmine' ],
        help = 'Application to run ILP on.')
...
parser.add_argument('--gen_tsv', action = 'store_true')
...
if 'kandan' == args.app:
    ...
    kandan.run(single_query = args.single_query,
        ...
        gen_tsv = args.gen_tsv, gen_cpp = args.gen_cpp,
        membound_factor = args.membound_factor,
        run_test_read_overall = False, output = old_stdout)
elif 'redmine' == args.app:
    # Load redmine and run.
    from repo.benchmark.redmine import redmine
    redmine.run(single_query = args.single_query,
        queries = args.queries,
        gen_tsv = args.gen_tsv, gen_cpp = args.gen_cpp,
        membound_factor = args.membound_factor,
        run_test_read_overall = False, output = old_stdout)
